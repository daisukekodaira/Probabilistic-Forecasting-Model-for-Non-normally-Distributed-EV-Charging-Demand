...
function [PICoverRate, MAPE, outTable] = getEVModel_MultipleDay(shortTermTable, predictorTable, targetTable)  
    % parameters
    ...
    [PICoverRate.ensemble, MAPE.ensemble] = display_result('EV charge (ensembled)', 'Demand [kwh]', energyTransPI, predData.EnsembleEnergy, targetTable.EnergyDemand, ci_percentage);
    [~, MAPE.kmeans] = display_result('EV charge (k-means)', 'Demand [kwh]',[], predData.IndEnergy(:,1), targetTable.EnergyDemand, ci_percentage);
    [~, MAPE.neuralNet] = display_result('EV charge (neural net)', 'Demand [kwh]',[], predData.IndEnergy(:,2), targetTable.EnergyDemand, ci_percentage);
